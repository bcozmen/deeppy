...
class SAC(BaseModel):
    ...
    def fast_log_normal(self,x, mu, var):
        return -0.5 * torch.log(2 * torch.pi * var) - (x - mu) ** 2 / (2 * var)
    ...
    def get_action(self, X):
        ...
        if self.mode == "discrete":
            ...
            x_t, mu, log_var = self.reparametrize(action_probs) #(Batch, dim)
            log_action_probs = self.fast_log_normal(x_t, mu, log_var.exp()).sum(1,keepdim=True)#(Batch, 1)
