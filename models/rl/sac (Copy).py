...
class Discrete_SAC(BaseModel):
    dependencies = [Network, DDQN]
    def __init__(self, ddqn_params, pnet_params, alpha_lr, gamma =0.99, target_entropy_coeff = 1. , device = None, criterion = nn.MSELoss()):
    ...
    def optimize(self, *X):
        states, actions, rewards, non_final_mask, non_final_next_states = self.ensure(*X)
